...
class PolylineMap : public IMapLoader {
 public:
  ...
  /// Replace the stored polylines and rebuild the query index.
  ///
  /// The non-const `map()` accessor leaves the index describing the polylines
  /// that used to be there, which fails as a silently empty local query rather
  /// than as an error. This does not.
  void SetMap(kitti::MapChunk chunk);
