...
///   1. Predict(T_curr_prev, MotionProcessCov(T_curr_prev, ...)) — motion
///      model from relative odometry
...
class LocalizationKF {
 public:
  ...
  /// Process noise for one step, sized by the motion that step contains.
  ///
  /// A fixed per-frame Q answers "how much less do I know after one frame?",
  /// which is not a question odometry error has an answer to: the error is
  /// produced by travelling, so the covariance has to grow with distance, not
  /// with time. Standing still adds only the stationary floor.
  ///
  /// The per-axis sigmas are stated in vehicle axes, where "along-track" and
  /// "heading" mean what they say; this rotates them into cam0 and then into
  /// the world frame the error state lives in, using the current attitude.
  /// That is why it is a member rather than a free function.
  ///
  /// @param T_curr_prev Same relative motion that Predict will be given.
  Mat66 MotionProcessCov(const Mat44& T_curr_prev,
                         const OdometryNoiseParams& params) const;
