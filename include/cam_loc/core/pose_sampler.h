...
/// Which cells of a grid to score, as half-extents in cells about the centre.
///
/// A negative half-extent means the whole axis. Cells outside the window are
/// left at `max_cost`: they cannot win, and their softmax weight is negligible,
/// so the volume its consumers read is unchanged in the region they read.
struct SearchWindow {
  int half_x = -1;
  int half_y = -1;
  int half_yaw = -1;

  bool IsWholeGrid() const { return half_x < 0 && half_y < 0 && half_yaw < 0; }
};
...
class PoseSampler {
 public:
  ...
  /// BEV.
  ///
  /// @warning Every pixel currently maps to the same BEV cell, so the result
  ///          is a single feature point rather than the perception outline.
  ...
  ///                      whole grid.
  /// @param window Cells to score; the default scores every cell.
  Status ComputeImageCosts(const kitti::MapChunk& map,
                           const Mat44& T_world_plane,
                           const LabelledDistanceTransform& dt, CostGrid& costs,
