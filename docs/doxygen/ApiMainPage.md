...
Two places reason in different axes and say so on the declaration.
`cam_loc::core::BevConfig` lays out its raster in the vehicle frame — X forward,
Y left — and `cam_loc::map::MapGeoref` starts from a local East/North plane.
Both convert at their own boundary, through `cam_loc::core::Frames`, so nothing
outside them sees anything but cam0.
