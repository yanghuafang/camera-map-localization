...
- [x] **The GPU path has now been run.** All three `CudaTest` parity tests pass on
...
- [ ] The host runs the `powersave` governor, which idles its cores at 1.2 GHz
      against a 3.3 GHz base. Setting `performance` needs root, so the CPU
      columns in [BENCHMARK.md](BENCHMARK.md) use best-of-N to pick a run whose
      clocks had come up. A number taken with the governor pinned would be
      firmer.
...
The corridor is now surveyed *wrong* on purpose, by a smooth 0.2 m error, so the
map and the world disagree and map matching has something to overcome. That
makes the accuracy figures mean something they did not before -- but the
corridor still follows the ground-truth path, so a systematic error in the
trajectory remains invisible, and the survey sigma is a number this project
chose rather than one a survey reported. An absolute accuracy claim still needs
a real surveyed map, which this dataset does not have.
...
- [ ] **`DistanceTransformCpu::Compute` allocates two whole-image scratch
      buffers per call.** `tmp` and `grid` hold one row or column at a time, so
      `max(width, height)` — 1241 floats at KITTI's resolution — would do where
      they take `width * height`. That is 3.7 MB value-initialized and thrown
      away per frame. Sizing them to the lane takes the kernel from 2.6 to
      2.5 ms and removes the last of the run-to-run spread — 8 runs of 8 within
      1.6%, against 13 of 15 — with all 113 tests still passing.

## Test coverage

`./scripts/coverage.sh` reports the current line coverage of `src/`. The
figure is not repeated here because every commit that adds a test moves it;
the gaps that matter are named instead, and those do not move on their own:
