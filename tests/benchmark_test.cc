...
TEST(BenchmarkTest, SmokeOracleCpuPasses) {
  ...
  // Ten frames is too few for the filter to settle onto the map's own
  // displacement, so this is looser than the suite's own gate. What it
  // asserts is that the case runs and lands near the map, not that it
  // beats a map it cannot beat.
  EXPECT_LT(result.summary.pose.rmse_translation_m, 0.30);
}
