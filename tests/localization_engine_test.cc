...
struct DriveResult {
  ...
  int first_frame_cells = 0;
};

DriveResult Drive(int frames, double step_m, double yaw_rate_rad,
                  cam_loc::LocalizationParams params);

double WorstPositionError(int frames, double step_m, double yaw_rate_rad) {
  return Drive(frames, step_m, yaw_rate_rad, TestParams()).worst_error_m;
}
...
DriveResult Drive(int frames, double step_m, double yaw_rate_rad,
  ...
  for (size_t f = 0; f < poses.size(); ++f) {
    ...
    out.worst_error_m = std::max(out.worst_error_m, err.norm());
    cells += engine.result().cells_searched;
    if (f == 0) out.first_frame_cells = engine.result().cells_searched;
  }
  out.mean_cells_searched = cells / static_cast<double>(poses.size());
  return out;
}
...
// Sizing the search from the covariance has to be free: the same drive, the
// same accuracy, fewer hypotheses.
TEST(LocalizationEngineTest, AdaptiveExtentTracksOnFewerHypotheses) {
  cam_loc::LocalizationParams fixed = TestParams();
  cam_loc::LocalizationParams adaptive = TestParams();
  adaptive.adaptive_extent.enabled = true;

  const auto a = Drive(40, 0.5, 0.0, fixed);
  const auto b = Drive(40, 0.5, 0.0, adaptive);

  EXPECT_LT(b.mean_cells_searched, a.mean_cells_searched)
      << "the window never narrowed";
  EXPECT_LT(b.worst_error_m, a.worst_error_m + 0.05)
      << "adaptive " << b.worst_error_m << " vs fixed " << a.worst_error_m;
}

// Before the filter has been corrected, its covariance is the prior's and says
// nothing about where the map match will land, so the first frames search wide.
TEST(LocalizationEngineTest, TheExtentStartsOpen) {
  cam_loc::LocalizationParams adaptive = TestParams();
  adaptive.adaptive_extent.enabled = true;
  const auto b = Drive(40, 0.5, 0.0, adaptive);
  EXPECT_EQ(b.first_frame_cells, adaptive.grid.TotalHypotheses());
}
