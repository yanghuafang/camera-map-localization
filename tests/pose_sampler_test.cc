...
// Consecutive aggregates reuse all but one of their frames, so the filter is
// handed the same evidence repeatedly. effective_frames() is what the
// measurement covariance is widened by to compensate.
TEST(CostAggregatorTest, EffectiveFramesCountsReusedEvidence) {
  cam_loc::AggregationParams ap;
  ap.window_size = 20;
  cam_loc::core::CostAggregator agg(ap);
  ...
  cam_loc::core::CostGrid current = grid;
  agg.Aggregate(current, cam_loc::Mat44::Identity(), 0.0);
  EXPECT_DOUBLE_EQ(agg.effective_frames(), 1.0) << "no history to reuse";

  double previous = 1.0;
  for (int f = 0; f < 15; ++f) {
    agg.PushHistory(grid, cam_loc::Mat44::Identity(), f, 0.0);
    current = grid;
    agg.Aggregate(current, cam_loc::Mat44::Identity(), 0.0);
    EXPECT_GE(agg.effective_frames(), previous);
    previous = agg.effective_frames();
  }

  // The 50/50 fuse caps the count: history can never outweigh the current
  // frame by more than one to one, however long the window.
  EXPECT_GT(previous, 3.0);
  EXPECT_LT(previous, 4.0);
}

namespace {

cam_loc::LocalizationParams SearchParams() {
  cam_loc::LocalizationParams params;
  params.grid.num_x = 11;
  params.grid.num_y = 11;
  params.grid.num_yaw = 5;
...
// A window restricts which cells are scored and nothing else: the cells inside
// carry exactly what the full search gives them, the rest stay at max_cost.
TEST(PoseSamplerTest, ASearchWindowScoresItsCellsAndNoOthers) {
  ...
  cam_loc::core::CostGrid full(params.grid);
  ASSERT_EQ(sampler.ComputeImageCosts(map, T_world, dt, full),
            cam_loc::Status::kOk);

  cam_loc::core::SearchWindow window;
  window.half_x = 2;
  window.half_y = 2;
  window.half_yaw = 1;
  cam_loc::core::CostGrid windowed(params.grid);
  ASSERT_EQ(sampler.ComputeImageCosts(map, T_world, dt, windowed, window),
            cam_loc::Status::kOk);

  int inside = 0;
  for (int iw = 0; iw < full.DimW(); ++iw) {
    for (int iy = 0; iy < full.DimY(); ++iy) {
      for (int ix = 0; ix < full.DimX(); ++ix) {
        const bool in = std::abs(ix - full.nx()) <= window.half_x &&
                        std::abs(iy - full.ny()) <= window.half_y &&
                        std::abs(iw - full.nw()) <= window.half_yaw;
        if (in) {
          EXPECT_FLOAT_EQ(windowed.At(ix, iy, iw), full.At(ix, iy, iw));
          ++inside;
        } else {
          EXPECT_FLOAT_EQ(windowed.At(ix, iy, iw), dt.max_cost);
        }
      }
    }
  }
  EXPECT_EQ(inside, 5 * 5 * 3);
}

TEST(PoseSamplerTest, TheDefaultWindowIsTheWholeGrid) {
  auto params = SearchParams();
  cam_loc::core::Projection proj(DefaultCalib());
  const cam_loc::Mat44 T_world = cam_loc::Mat44::Identity();
  const auto map = MakeStraightMap();
  ...
  cam_loc::core::CostGrid a(params.grid);
  cam_loc::core::CostGrid b(params.grid);
  ASSERT_EQ(sampler.ComputeImageCosts(map, T_world, dt, a),
            cam_loc::Status::kOk);
  ASSERT_EQ(sampler.ComputeImageCosts(map, T_world, dt, b,
                                      cam_loc::core::SearchWindow{}),
            cam_loc::Status::kOk);
