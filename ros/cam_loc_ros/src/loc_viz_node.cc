/// ROS2 playback node: step/continuous KITTI sequence through
/// LocalizationEngine.
///
/// Publishes map/perception/cost markers, GT/estimate paths, camera image, and
/// status. Playback flow mirrors run_sequence: BuildEgomotion →
/// ResolvePerception → ProcessFrame.
#include <chrono>
#include <exception>
#include <filesystem>
#include <memory>
...
sensor_msgs::msg::Image LoadGrayImageMsg(const std::string& path,
...
class CamLocVizNode : public rclcpp::Node {
 public:
  CamLocVizNode() : Node("cam_loc_viz") {
    ...
    perception_mode_ =
        declare_parameter<std::string>("perception_mode", "auto");
    ...
    // Playing by default. The keyboard controls below read the terminal, so a
    // node launched by a script -- or simply left unfocused -- was never sent
    // the R that starts playback, and RViz showed a single static frame with
    // no indication that it was waiting for one.
    continuous_ = declare_parameter<bool>("autoplay", true);
    ...
    map_pub_ = create_publisher<visualization_msgs::msg::MarkerArray>(
        "map_markers", 10);
    perception_pub_ = create_publisher<visualization_msgs::msg::MarkerArray>(
        "perception_markers", 10);
    cost_pub_ = create_publisher<visualization_msgs::msg::MarkerArray>(
        "cost_markers", 10);
    ...
    gt_pose_pub_ =
        create_publisher<geometry_msgs::msg::PoseStamped>("gt_pose", 10);
    est_pose_pub_ =
        create_publisher<geometry_msgs::msg::PoseStamped>("estimate_pose", 10);
  ...
  /// Loads the dataset and starts playback; returns false if the dataset is
  /// missing or unreadable.
  ///
  /// Separate from the constructor because it can fail and a constructor has no
  /// way to say so without an exception. Nothing is scheduled until it
  /// succeeds: the wall timer that drives playback is created at the end of
  /// this function, so a failed Init leaves an inert node for main() to drop.
  bool Init() {
    if (!LoadDataset()) {
      RCLCPP_ERROR(get_logger(), "Failed to load KITTI dataset");
      return false;
    }

    RCLCPP_INFO(get_logger(), "Loaded %zu poses for sequence %02d",
                poses_.size(), sequence_);
    RCLCPP_INFO(get_logger(), "Playback is %s (autoplay).",
                continuous_ ? "running" : "paused");
    // Started first: whether it attached depends on there being a terminal,
    // and only then is it worth advertising the key bindings.
    keyboard_.Start();
    if (keyboard_.attached()) {
      RCLCPP_INFO(get_logger(), "Keyboard controls (focus this terminal):");
      ...
      RCLCPP_INFO(get_logger(),
                  "No terminal on stdin, so the keyboard controls are off; "
                  "use the autoplay and playback_hz parameters instead.");
    }
    ...
    engine_->set_map_loader(map_loader_);
 ...
 private:
  bool LoadDataset() {
    ...
    if (cam_loc::kitti::LoadPosesFile(poses_path, poses_) !=
        cam_loc::Status::kOk) {
      RCLCPP_ERROR(get_logger(), "Cannot load poses: %s", poses_path.c_str());
      return false;
    }
    const std::string calib_path =
        cam_loc::kitti::ResolveCalibPath(kitti_root_, sequence_);
    if (cam_loc::kitti::ParseCalibrationFile(calib_path, calib_) !=
        cam_loc::Status::kOk) {
    ...
    return cam_loc::map::CreateMapLoader(map_opt, map_loader_) ==
           cam_loc::Status::kOk;
  }
  ...
  void ProcessUntil(int target_frame) {
    ...
    while (last_processed_ < target_frame && last_processed_ + 1 < end_frame_) {
      ...
      auto pmode = cam_loc::perception::PerceptionSource::kAuto;
      if (!ParsePerceptionMode(perception_mode_, &pmode)) {
        RCLCPP_ERROR(get_logger(), "Unknown perception_mode: %s",
                     perception_mode_.c_str());
        rclcpp::shutdown();
        return;
      }
      if (cam_loc::perception::ResolvePerception(
              pmode, perception_root_, sequence_, f, *map_loader_,
              map_query_radius_m_, *projection_, T_gt, {}, 1, perception,
      ...
      engine_->set_debug_capture(f == target_frame);
  ...
  void PublishFrame(int frame) {
    ...
    if (!engine_->debug_snapshot().valid) {
      engine_->set_debug_capture(true);
      ProcessUntil(frame);
    }

    const auto& dbg = engine_->debug_snapshot();
    auto map_markers = cam_loc_ros::BuildMapMarkers(dbg.local_map);
    stampMarkers(map_markers);
    map_pub_->publish(map_markers);

    auto perception_markers = cam_loc_ros::BuildPerceptionMarkers(
        dbg.perception, *projection_, dbg.T_world_plane);
    stampMarkers(perception_markers);
    perception_pub_->publish(perception_markers);

    auto cost_markers =
        cam_loc_ros::BuildCostArgminMarker(dbg, engine_->result());
    ...
    gt_pose.pose = cam_loc_ros::Mat44ToPose(
        poses_[static_cast<size_t>(frame)].T_world_cam0);
    ...
    oss << "frame " << frame << "/" << (end_frame_ - 1) << " match="
        << (engine_->result().sampling_measurement_applied ? "yes" : "no")
  ...
  void OnTimer() {
    ...
    if (cmd == cam_loc_ros::KeyboardInput::Command::kToggleContinuous) {
      ...
      RCLCPP_INFO(get_logger(), continuous_ ? "Continuous playback ON"
                                            : "Continuous playback PAUSED");
    }
...
int main(int argc, char** argv) {
  ...
  int status = 0;
  // Backstop only: this node and cam_loc both report failure by return value.
  // rclcpp itself still throws — on a malformed parameter override, say — and
  // terminating with no message would be worse than catching it here.
  try {
    auto node = std::make_shared<CamLocVizNode>();
    if (node->Init()) {
      rclcpp::spin(node);
    } else {
      status = 1;
