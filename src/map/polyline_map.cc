// Polyline map storage, JSON ingest, and optional uniform-grid spatial index
// for queries.
...
int64_t PackCell(int ix, int iy) {
  // Pack through unsigned. Grid indices go negative as soon as the map crosses
  // the world origin, and shifting a negative *signed* value is undefined --
  // which the sanitizer says out loud and an ordinary build does not. What is
  // wanted here is the bit pattern, not the arithmetic, so the conversion to
  // unsigned is the operation rather than a way around the warning.
  const uint64_t hi = static_cast<uint64_t>(static_cast<uint32_t>(ix)) << 32;
  return static_cast<int64_t>(hi | static_cast<uint32_t>(iy));
}
...
void PolylineMap::QueryPolylineIndices(const Vec3& query, double radius_m,
  ...
  const int ix0 =
      static_cast<int>(std::floor((query.x() - radius_m) / cell_size_m_));
  const int ix1 =
      static_cast<int>(std::floor((query.x() + radius_m) / cell_size_m_));
  const int iy0 =
      static_cast<int>(std::floor((query.y() - radius_m) / cell_size_m_));
  const int iy1 =
      static_cast<int>(std::floor((query.y() + radius_m) / cell_size_m_));
  for (int ix = ix0; ix <= ix1; ++ix) {
    for (int iy = iy0; iy <= iy1; ++iy) {
  ...
  for (const auto& item : j["polylines"]) {
    ...
    if (item.contains("points") && item["points"].is_array()) {
      const bool wgs84_points =
          have_georef &&
          item.value("coord_frame", std::string("world")) == "wgs84";
...
void PolylineMap::SetMap(kitti::MapChunk chunk) {
  map_ = std::move(chunk);
  RebuildSpatialIndex();
}
