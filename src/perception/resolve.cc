...
/// Project what a camera at @p T_world would see.
///
/// Reads `map_loader.world()`, not the map. A camera sees the world; the map is
/// a survey of it, and the two differ. Projecting the *map* here instead would
/// put the survey's error on both sides of the match, where it cancels exactly
/// -- the search would recover the true pose however wrong the map was, and no
/// accuracy number taken that way would mean anything. For a real map the two
/// are the same object and this costs nothing.
Status SynthesizeAt(const map::IMapLoader& map_loader, double map_radius_m,
  ...
  if (map_loader.world().QueryLocalMap(T_world, map_radius_m, local) !=
          Status::kOk ||
