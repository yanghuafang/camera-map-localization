/// Eval summary rollups: matching quality rates, filter consistency, and
...
MapMatchingQuality SummarizeMatchingQuality(
  ...
  for (const auto& r : records) {
    ...
    sum_margin += r.mode_margin;
    sum_cells += r.cells_searched;
    if (r.num_cost_modes >= 2) ++multimodal;
  ...
  q.mean_mode_margin = sum_margin / n;
  q.mean_cells_searched = sum_cells / n;
  q.multimodal_rate = static_cast<double>(multimodal) / n;
...
SequenceEvalSummary SummarizeEval(const std::vector<FrameEvalRecord>& records) {
  ...
  std::vector<FrameConsistency> consistency;
  consistency.reserve(records.size());
  for (const auto& r : records) {
    pose_errors.push_back(r.pose_error);
    frame_ms.push_back(r.frame_ms);
    consistency.push_back(r.consistency);
  }
  out.pose = SummarizeErrors(pose_errors);
  out.consistency = SummarizeConsistency(consistency);
