...
Status RunSequenceEval(const std::vector<Pose>& poses, const Calibration& calib,
  ...
  for (int f = start; f < end; ++f) {
    ...
    ego = AddEgomotionNoise(ego, config.ego_noise, config.ego_noise_seed);
    ...
    // Only a frame the filter has actually published carries a covariance
    // worth testing; before initialization there is nothing to be consistent
    // with.
    if (engine.result().valid) {
      rec.consistency = PoseNees(engine.result().T_world_rig, T_gt,
                                 engine.result().covariance);
    }
    ...
    rec.mode_margin = engine.result().mode_margin;
    rec.num_cost_modes = engine.result().num_cost_modes;
    rec.cells_searched = engine.result().cells_searched;
