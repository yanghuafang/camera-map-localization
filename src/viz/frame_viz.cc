...
Status RenderFrameViz(const FrameVizInput& input, const std::string& output_dir,
  ...
  meta["num_cost_modes"] = input.result->num_cost_modes;
  meta["mode_margin"] = input.result->mode_margin;
  meta["second_mode_xyyaw"] = {input.result->second_mode_xyyaw.x(),
                               input.result->second_mode_xyyaw.y(),
                               input.result->second_mode_xyyaw.z()};
