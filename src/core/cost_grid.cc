...
Vec3 CostGrid::RefinedOffset(const ArgMinResult& argmin) const {
  ...
  // Step to the minimum of the local quadratic: delta = -H^-1 g. Unlike three
  // independent parabolas this sees the cross terms, which is what a basin
  // lying at an angle to the grid axes is made of -- and lane geometry puts one
  // there whenever the road is not aligned with the grid.
  const LocalQuadratic fit = FitLocalQuadratic(*this, argmin);
  if (fit.valid) {
    const Eigen::SelfAdjointEigenSolver<Eigen::Matrix3d> es(fit.hessian);
    if (es.info() == Eigen::Success) {
      const Vec3& curvature = es.eigenvalues();
      const double sharpest = curvature.maxCoeff();
      const Vec3 gradient = es.eigenvectors().transpose() * fit.gradient;

      // Step in proportion to how well each direction is determined. The
      // basin is extremely anisotropic -- the along-track curvature is under a
      // thousandth of the sharpest -- so an undamped -g/lambda along it is
      // enormous, chases noise, and drags the other axes with it through the
      // cross terms. Adding a fraction of the sharpest curvature to every
      // eigenvalue leaves the well-determined directions almost untouched and
      // shrinks the step along the flat one towards the cell centre, which is
      // the MAP step under a prior that the argmin is already close.
      constexpr double kDamping = 0.01;
      const double damping = kDamping * std::max(sharpest, 0.0);
      Vec3 step_eigen = Vec3::Zero();
      for (int i = 0; i < 3; ++i) {
        const double denominator = curvature[i] + damping;
        if (denominator > 0.0) step_eigen[i] = -gradient[i] / denominator;
      }

      Vec3 step = es.eigenvectors() * step_eigen;
      // Half a cell each way: the quadratic is fitted from immediate
      // neighbours and says nothing further out, and the winning cell is by
      // definition the nearest one to the true minimum.
      const Vec3 limit(0.5 * step_x_, 0.5 * step_y_, 0.5 * step_yaw_);
      if (step.allFinite() && !step.isZero()) {
        for (int i = 0; i < 3; ++i) {
          step[i] = std::clamp(step[i], -limit[i], limit[i]);
        }
        return centre + step;
      }
    }
  ...
  // Border cell, or a fit with no minimum to step to: fall back to one
  // parabola per axis, which needs only three collinear samples.
