...
void Edt1d(const std::vector<float>& f, std::vector<float>& d, int n) {
  // 1D squared distance transform (Felzenszwalb & Huttenlocher, O(n)). Each
  // sample q defines a parabola (q' - q)^2 + f[q]; the result is their lower
  // envelope sampled at every q'. Phase 1 sweeps left→right building that
  // envelope; phase 2 reads it back:
  ...
  // Phase 1: intersect each new parabola with the envelope, popping any it now
  // dominates.
  for (int q = 1; q < n; ++q) {
  ...
  // Phase 2: for each position, advance to the parabola whose interval covers
  // it and evaluate.
  k = 0;
  for (int q = 0; q < n; ++q) {
    while (z[k + 1] < q) ++k;
...
Status DistanceTransformCpu::Compute(const std::vector<uint8_t>& binary,
  ...
  // Seed the transform: feature pixels (binary == 0) start at distance 0,
  // everything else at a large "infinity" so the two separable passes below
  // propagate real distances into it.
  for (int i = 0; i < n; ++i) {
...
Status DistanceTransformCpu::RasterizePolylines(
    const std::vector<Vec2>& points, int width, int height, float stroke_width,
