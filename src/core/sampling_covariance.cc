...
SamplingConfidence SamplingCovariance::Compute(
    ...
    float cost_scale) {
  // Turns the shape of the cost surface into a measurement covariance for the
  // EKF: a sharp, isolated minimum yields a small covariance (trust the map
  // match) while a broad or flat basin yields a large one. It is a
  // softmax-weighted second moment of the offset cloud about the argmin — a
  // cheap confidence proxy, not the true cost Hessian.
  ...
  for (int iw = 0; iw < grid.DimW(); ++iw) {
    for (int iy = 0; iy < grid.DimY(); ++iy) {
      for (int ix = 0; ix < grid.DimX(); ++ix) {
        ...
        const double w =
            std::exp(-static_cast<double>(c - argmin.cost) / cost_scale);
        if (w < 1e-6) continue;
