...
std::vector<BenchmarkCase> DefaultBenchmarkSuite(const std::string& data_root) {
  const std::string smoke = data_root + "/smoke_kitti";
  const std::string kitti = data_root + "/kitti_odometry";
  const std::string perception = data_root + "/perception";
...

  {
    ...
    c.thresholds.max_rmse_translation_m = 0.45;
    c.thresholds.max_rmse_yaw_deg = 0.5;
    c.thresholds.min_match_rate = 0.90;
  ...
  }
  {
    ...
    c.thresholds.max_rmse_translation_m = 0.35;
    c.thresholds.max_rmse_yaw_deg = 1.5;
...
bool CheckThresholds(const BenchmarkCase& spec,
                     const cam_loc::kitti::SequenceEvalSummary& summary,
                     std::string& out_reason) {
  std::vector<std::string> fails;
  const auto& t = spec.thresholds;
  if (summary.pose.rmse_translation_m > t.max_rmse_translation_m) {
    fails.push_back("rmse_translation_m=" +
                    std::to_string(summary.pose.rmse_translation_m) + " > " +
                    std::to_string(t.max_rmse_translation_m));
  }
  if (summary.pose.rmse_yaw_deg > t.max_rmse_yaw_deg) {
    fails.push_back(
        "rmse_yaw_deg=" + std::to_string(summary.pose.rmse_yaw_deg) + " > " +
        std::to_string(t.max_rmse_yaw_deg));
  }
  if (summary.matching.match_rate < t.min_match_rate) {
    fails.push_back(
        "match_rate=" + std::to_string(summary.matching.match_rate) + " < " +
...
Status RunBenchmarkCase(const BenchmarkCase& spec, BenchmarkResult& out) {
  ...
  if (kitti::RunSequenceEval(poses, calib, map_loader, cfg, records) !=
      Status::kOk) {
...
Status RunBenchmarkSuite(const std::vector<BenchmarkCase>& cases,
                         BenchmarkSuiteResult& out) {
