/// CLI: full sequence evaluation with pose, consistency and matching metrics,
/// and an optional per-frame CSV.
#include <exception>
#include <fstream>
#include <iostream>
#include <memory>
...
struct Options {
  ...
  double noise_px = 0.0;
  double map_error_m = -1.0;  // <0 keeps the map's own survey error
  double noise_point_dropout = 0.0;
  double noise_polyline_dropout = 0.0;
  uint32_t noise_seed = 1;
  double ego_scale_error = 0.0;
  double ego_scale_sigma = 0.0;
  double ego_heading_bias = 0.0;
  double ego_heading_sigma = 0.0;
  uint32_t ego_noise_seed = 2;
  float cost_softmax_scale = 0.5f;
  int aggregation_window = 12;
};
  ...
  *ok = true;
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--kitti-root") {
      ...
      need_number("--map-error-m", cam_loc::apps::ParseDouble,
                  &opt.map_error_m);
    ...
    } else if (arg == "--ego-scale-error") {
      need_number("--ego-scale-error", cam_loc::apps::ParseDouble,
                  &opt.ego_scale_error);
    } else if (arg == "--ego-scale-sigma") {
      need_number("--ego-scale-sigma", cam_loc::apps::ParseDouble,
                  &opt.ego_scale_sigma);
    } else if (arg == "--ego-heading-bias") {
      need_number("--ego-heading-bias", cam_loc::apps::ParseDouble,
                  &opt.ego_heading_bias);
    } else if (arg == "--ego-heading-sigma") {
      need_number("--ego-heading-sigma", cam_loc::apps::ParseDouble,
                  &opt.ego_heading_sigma);
    } else if (arg == "--ego-noise-seed") {
      need_number("--ego-noise-seed", cam_loc::apps::ParseUint32,
                  &opt.ego_noise_seed);
      ...
      std::cout
          ...
          << "  --cost-softmax-scale F  Sampling covariance scale (default "
             "0.5)\n"
          << "  --aggregation-window N  Temporal window (default 12)\n"
          << "  --cost-threads N        CPU pose-grid threads (0 = "
             "hardware)\n"
          << "  --adaptive-extent       Size the searched region from the "
             "filter covariance\n"
          << "  --ego-scale-error F     Odometry scale bias, fraction of "
             "distance\n"
          << "  --ego-scale-sigma F     Random part of the same, per step\n"
          << "  --ego-heading-bias F    Odometry heading drift, deg per metre\n"
          << "  --ego-heading-sigma F   Random part of the same, deg per "
             "metre\n"
...
int main(int argc, char** argv) {
  try {
    ...
    if (opt.map_error_m >= 0.0) {
      map_opt.survey_error.lateral_sigma_m = opt.map_error_m;
      map_opt.survey_error.longitudinal_sigma_m = opt.map_error_m;
    }
    ...
    cfg.ego_noise_seed = opt.ego_noise_seed;
    cfg.ego_noise.scale_error = opt.ego_scale_error;
    cfg.ego_noise.scale_sigma = opt.ego_scale_sigma;
    cfg.ego_noise.heading_bias_deg_per_m = opt.ego_heading_bias;
    cfg.ego_noise.heading_sigma_deg_per_m = opt.ego_heading_sigma;
    ...
    if (!opt.output_csv.empty()) {
      ...
      csv << "frame,translation_m,lateral_m,longitudinal_m,vertical_m,"
             "yaw_deg,min_cost,cost_spread,match,flat,synth,offset_m,nees,"
             "mode_margin,num_modes,cells\n";
    }
    ...
    for (const auto& r : records) {
      if (csv) {
        csv << r.frame << "," << r.pose_error.translation_m << ","
            ...
            << r.best_offset_m << ",";
        // A frame with no verdict is left blank rather than written as 0,
        // which would read as a perfectly consistent frame.
        if (r.consistency.valid) csv << r.consistency.nees;
        csv << "," << r.mode_margin << "," << r.num_cost_modes << ","
            << r.cells_searched << "\n";
      }
    }
    ...
    std::cout << "Yaw RMSE (deg):            " << summary.pose.rmse_yaw_deg
              << "\n";
    std::cout << "NEES mean:                 " << summary.consistency.mean_nees
              << " (expected " << cam_loc::kitti::kPoseErrorDof << ")\n";
    std::cout << "ANEES:                     " << summary.consistency.anees
              << "\n";
    std::cout << "Within 95% bound:          "
              << (100.0 * summary.consistency.within_95_rate) << "% of "
              << summary.consistency.num_frames << " frames (expected 95%)\n";
    ...
    std::cout << "Mean mode margin:          "
              << summary.matching.mean_mode_margin << "\n";
    std::cout << "Mean cells searched:       "
              << summary.matching.mean_cells_searched << "\n";
    std::cout << "Multimodal rate:           "
              << (100.0 * summary.matching.multimodal_rate) << "%\n";
