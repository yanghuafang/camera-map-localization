/// CLI: render offline PNG debug panels for one frame (and optional trajectory
/// overlay).
#include <exception>
#include <filesystem>
#include <iostream>
  ...
  *ok = true;
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--kitti-root") {
      ...
      need_number("--frame", cam_loc::apps::ParseInt, &opt.frame);
      ...
      std::cout
          ...
          << "  Render map, perception, DT, cost grid, and pose for one frame "
             "(or trajectory).\n"
          << "  --kitti-root PATH       KITTI root (poses + calib; images "
             "optional)\n"
          ...
          << "  --trajectory            Also write trajectory_gt_est.png for "
             "processed frames\n"
...
int main(int argc, char** argv) {
  try {
    ...
    if (cam_loc::map::CreateMapLoader(map_opt, map_loader) !=
        cam_loc::Status::kOk) {
    ...
    cam_loc::perception::PerceptionSource pmode =
        cam_loc::perception::PerceptionSource::kAuto;
    if (!ParsePerceptionMode(opt.perception_mode, &pmode)) return 2;

    const int last_frame =
        opt.max_frames < 0
            ? opt.frame
            : std::min(opt.max_frames - 1, static_cast<int>(poses.size()) - 1);
    if (opt.frame < 0 || opt.frame >= static_cast<int>(poses.size()) ||
        last_frame < 0) {
    ...
    for (int f = 0; f <= last_frame; ++f) {
      ...
      engine.set_debug_capture(f == opt.frame);
    ...
    if (!engine.debug_snapshot().valid) {
    ...
    viz_in.debug = &engine.debug_snapshot();
    viz_in.projection = &projection;
    viz_in.result = &engine.result();
    viz_in.gt_pose = &poses[static_cast<size_t>(opt.frame)];

    const std::string image_path = cam_loc::kitti::ResolveImagePath(
        opt.kitti_root, opt.sequence, opt.frame);
    if (std::filesystem::is_regular_file(image_path)) {
      if (cam_loc::viz::LoadRgbImage(image_path, viz_in.camera) !=
          cam_loc::Status::kOk) {
    ...
    if (cam_loc::viz::RenderFrameViz(viz_in, opt.output_dir, viz_out) !=
        cam_loc::Status::kOk) {
      std::cerr << "renderFrameViz failed\n";
      return 1;
    }

    std::cout << "Wrote visualization for frame " << opt.frame << " to "
              << opt.output_dir << ":\n";
    ...
    if (opt.trajectory) {
      ...
      if (cam_loc::viz::RenderTrajectoryViz(traj, traj_path) !=
          cam_loc::Status::kOk) {
